...
public interface ISellAndBuyStock {
     ...
     */
    StockSolution findBestSolutionTransactMutilTimes(List<StockMetadata> stockList);

    /**
     * 按leetcode中定义的传入参数和返回结果，可以交易多次，找出股票买卖的最大利润
     * @param prices 预设的股票价格变化趋势数据
     * @return 返回利润最大的买卖方案，如果没有利润就返回0
     */
    int findBestSolutionTransactMutilTimes(int[] prices);

    /**
     * 按leetcode中定义的传入参数和返回结果，只能交易一次，找出股票买卖的最大利润
     * @param prices 预设的股票价格变化趋势数据
     * @return 返回利润最大的买卖方案，如果没有利润就返回0
     */
    int findBestSolutionTransactSingleTime(int[] prices);
}
